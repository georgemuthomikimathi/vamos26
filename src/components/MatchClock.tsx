...
import { formatMatchMinute } from "@/lib/scores/types";
...
export default function MatchClock({ match, size = "sm" }: MatchClockProps) {
  ...
  const showCountdown =
    match.status === "scheduled" && untilKickoff != null && untilKickoff > 0 && untilKickoff < 24 * 60 * 60_000;
  ...
  if (showCountdown) {
    return (
      <div className={`text-center ${lg ? "py-2" : ""}`}>
        <p className={`uppercase tracking-widest text-pitch font-semibold ${lg ? "text-xs" : "text-[10px]"}`}>
          Kickoff in
        </p>
